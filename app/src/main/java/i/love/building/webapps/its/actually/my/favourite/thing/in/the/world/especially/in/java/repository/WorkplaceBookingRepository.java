...
public interface WorkplaceBookingRepository extends JpaRepository<WorkplaceBooking, Long> {
    ...
    @Query(
            "select b from WorkplaceBooking b where b.user.id = :user_id and b.bookedDate ="
                    + " :booked_date")
    Optional<WorkplaceBooking> findByUserIdAndDate(
            @Param("user_id") Long userId, @Param("booked_date") LocalDate date);

    @Query(
            "select b from WorkplaceBooking b where b.workplace.id = :workplace_id and b.bookedDate"
                    + " = :booked_date")
    Optional<WorkplaceBooking> findByWorkplaceAndDate(
            @Param("workplace_id") Long workplaceId, @Param("booked_date") LocalDate date);
