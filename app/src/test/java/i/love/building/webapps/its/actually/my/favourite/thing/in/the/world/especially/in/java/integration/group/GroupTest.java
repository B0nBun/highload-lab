...
public class GroupTest extends IntegrationTest {
    ...
    @BeforeAll
    void setup() {
        User user = new User("group-test-user", BCrypt.hashpw("", BCrypt.gensalt()), User.Role.admin);
    ...
    void canCreate() throws Exception {
        ...
        var createReq = MockMvcRequestBuilders.post("/api/group/")
            .content("""
    {"name": "test-group-name"}
        ...
        this.mockMvc.perform(getAllReq).andExpect(MockMvcResultMatchers.status().isOk())
            .andExpect(MockMvcResultMatchers.jsonPath("$[0].name", Matchers.is("test-group-name")));
    }
    ...
    void canAddAndDeleteUsers() throws Exception {
        ...
        this.mockMvc.perform(getReq).andExpect(MockMvcResultMatchers.status().isOk())
            .andExpect(MockMvcResultMatchers.jsonPath("$.userIds", Matchers.hasSize(0)));
        
        var addUserReq = MockMvcRequestBuilders.post(String.format("/api/group/%d/users", id))
            .contentType(MediaType.APPLICATION_JSON)
        ...
        this.mockMvc.perform(getReq).andExpect(MockMvcResultMatchers.status().isOk())
            .andExpect(MockMvcResultMatchers.jsonPath("$.userIds[0]", Matchers.is(testUser.getId().intValue())));

        var removeUserReq = MockMvcRequestBuilders.delete(String.format("/api/group/%d/users/%d", id, this.testUser.getId()));
    ...
    void canAddAndDeleteOffices() throws Exception {
        ...
        var addReq = MockMvcRequestBuilders.post(String.format("/api/group/%d/offices", id))
            .contentType(MediaType.APPLICATION_JSON)
            .content(String.format("""
                    {"officeId": %d}
        ...
        this.mockMvc.perform(getReq).andExpect(MockMvcResultMatchers.status().isOk())
            .andExpect(MockMvcResultMatchers.jsonPath("$.officeIds[0]", Matchers.is(this.testOffice.getId().intValue())));

        var removeReq = MockMvcRequestBuilders.delete(String.format("/api/group/%d/offices/%d", id, this.testOffice.getId()));
    ...
    void canDelete() throws Exception {
        ...
        this.mockMvc.perform(getOneReq).andExpect(MockMvcResultMatchers.status().isOk())
            .andExpect(MockMvcResultMatchers.jsonPath("$.name", Matchers.is("test-group-name")));
