...
public class WorkplaceTest extends IntegrationTest {
    ...
    void testGetCreateUpdateDelete() throws Exception {
        ...
        var createReq = MockMvcRequestBuilders.post("/api/workplace/")
    ...
    {"officeId": %d,"monitors": 2, "audioEquipment": "absent"}
        ...
        MvcResult result = this.mockMvc.perform(getAllReq).andExpect(MockMvcResultMatchers.status().isOk())
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$[0].audioEquipment", Matchers.is("absent")))
            .andReturn();

        String response = result.getResponse().getContentAsString();
        Integer id = JsonPath.parse(response).read("$[0].id");

        var updateReq = MockMvcRequestBuilders.put(String.format("/api/workplace/%d", id))
            .contentType(MediaType.APPLICATION_JSON)
            .content("""
        ...
        this.mockMvc.perform(getOneReq).andExpect(MockMvcResultMatchers.status().isOk())
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.audioEquipment", Matchers.is("headset")));
